...
// Set ALLOWED_ORIGIN (supabase secrets set ALLOWED_ORIGIN=https://yourapp.com) in
// production to stop other websites from calling this function with a stolen/replayed
// browser session. Falls back to "*" so local dev keeps working out of the box.
const cors = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create") {
      ...
      // "admin" is intentionally NOT creatable here — admin is always the business
      // owner (businesses.owner_id); business_members only ever holds cashier/manager.
      if (!["cashier", "manager"].includes(role)) return json({ error: "Invalid role" }, 400);
